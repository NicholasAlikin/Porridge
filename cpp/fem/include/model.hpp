...
struct ModelTraits {
	...
	static void do_assemble_nonlinear(const Model& model, const Assemble& assemble
                                    ,       V&       matrix
                                    ,       math::vector<double>&       load
...
private:
	...
	mutable math::vector_t<double,2> elem_matrix;			// element local matrix
	mutable math::vector_t<double,2> elem_matrix2;			// element local matrix second (to assemble 2 matrices simulteneously)
	mutable math::vector_t<double,1> elem_load;				// element local load vector
	mutable math::vector_t<double,1> elem_displ;			// element local displacement vector
	mutable math::vector_t<double,1> elem_vel;			// element local velocity vector
	mutable math::vector_t<double,1> elem_accel;			// element local acceleration vector
public:
...
requires std::same_as<typename V::basic_value_type, double>
&& (math::Vector<V>	|| math::Matrix<V>)
void ModelTraits::do_assemble_nonlinear(const Model& model, const Assemble& assemble		 /* Model and Assemble objects */
...
requires std::same_as<typename V::basic_value_type, double>
&& (math::Vector<V>	|| math::Matrix<V>)
void ModelTraits::do_assemble_nonlinear(const Model& model, const Assemble& assemble
                                ,       V&       matrix
                                ,       math::vector<double>&       load
                                , const math::vector<double>&       q
			                    , const math::vector<double>&       dqdt
